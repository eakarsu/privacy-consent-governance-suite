const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4720';
...
const adminCookie = await login('admin@privacy-consent.local', 'admin123');
const managerCookie = await login('manager@privacy-consent.local', 'manager123');
const analystCookie = await login('analyst@privacy-consent.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/consents', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/consents', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/consents/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/consents/approve`, {
  method: 'POST',
  headers: {
